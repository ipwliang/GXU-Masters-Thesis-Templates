...
\begin{figure}[htb]
    \centering
    \subfloat{
    \includegraphics[width=1.0\linewidth]{../figure/nyu2.png}
    }
    \captionsetup{font=footnotesize}
    \bicaption{NYU2 数据集}{NYU2 Dataset}
    \label{fig:nyu2}
\end{figure}
...
\begin{figure}[htb]
    \centering
    \subfloat{
    \includegraphics[width=1.0\linewidth]{../figure/dense_haze.png}
    }
    \captionsetup{font=footnotesize}
    \bicaption{Dense-Haze 数据集}{Dense-Haze Dataset}
...
%     \centering
%     \subfloat[NYU2 GT]{
%     \includegraphics[width=0.13\linewidth]{../figure/dense_haze.png}
%     }
%     \subfloat[NYU2 hazy]{
%     \includegraphics[width=0.13\linewidth]{../figure/dense_haze.png}
%     }
...
%     }
%     \captionsetup{font=footnotesize}
%     \bicaption{不同模型在 NYU2 数据集上的推理结果}{Dense-Haze Dataset}
%     \label{fig:de_nyu2}
% \end{figure}

% \begin{figure}[htb]
%     \centering
%     \subfloat[Dense-Haze GT]{
%     \includegraphics[width=0.13\linewidth]{../figure/dense_haze.png}
%     }
%     \subfloat[Dense-Haze hazy]{
%     \includegraphics[width=0.13\linewidth]{../figure/dense_haze.png}
%     }
%     \subfloat[AOD-Net]{
%     \includegraphics[width=0.13\linewidth]{../figure/dense_haze.png}
...
%     }
%     \captionsetup{font=footnotesize}
%     \bicaption{不同模型在 Dense-Haze 数据集上的推理结果}{Dense-Haze Dataset}
%     \label{fig:de_dense_haze}
% \end{figure}

% 图 \ref{fig:de_nyu2} 和图 \ref{fig:de_dense_haze} 展示了 AODNet、FFA-Net、GCANet 和 our 模型在 NYU2 数据集和 Dense-Haze 数据集上的推理结果。
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.16\textwidth}p{0.15\textwidth}p{0.16\textwidth}p{0.13\textwidth}p{0.13\textwidth}p{0.13\textwidth}}
        \toprule
...
\begin{figure}[h]
    \centering
        \subfloat[CGF-YOLO\label{fig:fogtt100k_ex_f1}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_ex_F1_curve.png}
        }
        \subfloat[YOLOv11s\label{fig:fogtt100k_11s_f1}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_11_F1_curve.png}
        } \\
        \subfloat[YOLOv10s\label{fig:fogtt100k_10s_f1}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_10_F1_curve.png}
        }
        \subfloat[YOLOv9s\label{fig:fogtt100k_9s_f1}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_9_F1_curve.png}
        }
    \captionsetup{font=footnotesize}
...
表 \ref{tab:compare_studies_fogtt100k} 展示了 our 模型和 YOLOv11s、YOLOv10s、YOLOv9s 在 FOG-TT100K 数据集上的测试结果。
...
\begin{figure}[H]
    \centering
        \subfloat[CGF-YOLO\label{fig:fogtt100k_ex_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_ex_confusion_matrix_normalized.png}
        }
        \subfloat[YOLOv11s\label{fig:fogtt100k_11s_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_11_confusion_matrix_normalized.png}
        } \\
        \subfloat[YOLOv10s\label{fig:fogtt100k_10s_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_10_confusion_matrix_normalized.png}
        }
        \subfloat[YOLOv9s\label{fig:fogtt100k_9s_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogtt_9_confusion_matrix_normalized.png}
        }
    \captionsetup{font=footnotesize}
...
图 \ref{fig:fogtt100k_f1} 展示了不同模型的F1得分曲线。our 模型的mAP是最高的（0.592），这与其高F1 得分一致。 our 模型的F1 得分高于YOLOv9s、YOLOv11s和YOLOv10s。 这表明，our 模型在平均精度上好于其他YOLO模型。 F1曲线的高性能与our模型的精度和召回率之间的良好平衡有关。 
...
\begin{figure}[H]
    ...
    \subfloat[CGF-YOLO\label{fig:fogvd_ex_f1}]{
        \includegraphics[width=0.4\textwidth]{../figure/fogvd_ex_F1_curve.png}
    }
...
在表 \ref{tab:compare_studies_fogvd} 中，在精确度（P）的评估中，our 模型以0.721的显著成绩领先，相较于YOLOv11s的0.631高14.3\%，超出YOLOv10s的0.674的7.0\%，同时比YOLOv9s的0.642高12.3\%。这一结果表明our 模型在降低假阳性率方面具有显著优势，能够更精准地识别雾天环境下的交通目标，减少误报情况。通过对模型的深度特征提取网络进行了优化，有效提升了对雾天低对比度目标的判别能力。

尽管召回率（R）仅为0.074，但已显著超越YOLOv11s的0.049、YOLOv10s的0.054和YOLOv9s的0.055。这表明our 模型在雾天复杂场景中能够检测到更广泛的交通目标，尽管因受雾天目标可见性限制整体数值较低，但相对其他模型仍具有明显的检测范围优势。通过改进优化，our 模型能够动态分配计算资源，优先处理雾天场景中的高信号区域，从而在有限的检测范围内捕捉到更多真实目标。

\begin{figure}[h]
    \centering
        \subfloat[CGF-YOLO\label{fig:fogvd_ex_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogvd_ex_confusion_matrix_normalized.png}
        }
        \subfloat[YOLOv11s\label{fig:fogvd_11s_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogvd_v11s_confusion_matrix_normalized.png}
        } \\
        \subfloat[YOLOv10s\label{fig:fogvd_10s_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogvd_v10s_confusion_matrix_normalized.png}
        }
        \subfloat[YOLOv9s\label{fig:fogvd_9s_cmn}]{
            \includegraphics[width=0.45\textwidth]{../figure/fogvd_v9s_confusion_matrix_normalized.png}
        }
    \captionsetup{font=footnotesize}
...
\begin{figure}[H]
    \centering
        \subfloat[FOG-TT100K 原图 \label{fig:fogtt_show_1}]{
            \includegraphics[width=1.0\textwidth]{../figure/tt_0_0.png}
        }
        \\
        \subfloat[CGF-YOLO \label{fig:fogtt_ex_1}]{
            \includegraphics[width=1.0\textwidth]{../figure/fogtt_0_0_ex.png}
        }
        \\
        \subfloat[YOLOv11s \label{fig:fogtt_11s_1}]{
            \includegraphics[width=1.0\textwidth]{../figure/fogtt_0_0_11.png}
        }
        \\
        \subfloat[YOLOv10s \label{fig:fogtt_10s_1}]{
            \includegraphics[width=1.0\textwidth]{../figure/fogtt_0_0_10.png}
        }
        \\
        \subfloat[YOLOv9s \label{fig:fogtt_9s_1}]{
            \includegraphics[width=1.0\textwidth]{../figure/fogtt_0_0_9.png}
        }
    \captionsetup{font=footnotesize}
...
图\ref{fig:fogtt_1}显示，由于目标场景的复杂性，所有模型都无法完整并准确地进行识别和检测。在第1张图中，CGF-YOLO和YOLOv10s都检测到了目标，但其他模型均未发现这一目标。在第3张图中，有2个目标标签，CGF-YOLO模型检测到了这2个目标，其他模型只检测到了其中1个目标。在第4张图中，有2个目标标签，所有模型都只发现了其中1个目标。可以发现，CGF-YOLO模型对于目标分布不均匀的场景中能有更好的表现。有利于在目标分布不均匀的无人机监测场景下识别出更多目标。
...
根据图 \ref{fig:fogtt_1}、图 \ref{fig:fogvd_1}，当目标分布不均匀且分布不密集时，CGF-YOLO可以实现更好的识别性能，并在目标分布密集的条件下正确捕获更多目标。这有利于无人机在面对复杂场景时识别小目标。
